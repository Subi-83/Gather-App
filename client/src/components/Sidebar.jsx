    ...
    <div className={`bg-[#29bdc4]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : '' }`}>
      ...
      <div className='pb-5'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5 '>
            ...
            <input type="text" onChange={(e)=>setInput(e.target.value)} className='bg-transparent border-none outline-none text-white text-xs placeholder:text-[#c8c8c8] flex-1 ' placeholder='Search User....' />
        </div>
      ...
      <div className='flex flex-col'> 
        ...
        {filteredUsers.map((user,index) => (
            <div onClick={()=>{setSelectedUser(user)}} key = {index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id ? 'bg-[#29bdc4]/60 scale-[1.02]' : 'hover:bg-[#29bdc4]/30'}`} >
                ...
                {unseenMessages?.[user._id] > 0 && <p className='absolute top-4 right-0 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-pink-700/50'>
