    ...
    <div className={`bg-[#29bdc4]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : '' }`}>
      ...
      <div className='flex flex-col'> 
        ...
        {userDummyData.map((user,index) => (
            <div onClick={()=>{setSelectedUser(user)}} key = {index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && ' hover:bg-[#29bdc4]/60 hover:scale-[1.02]'}`} >
                ...
                {
                    index > 2 && <p className='absolute top-4 right-0 text-xs h-5 w-5 flex justify-center items-center rounded-full bg-pink-700/50'>
                        {index}
                    </p>
